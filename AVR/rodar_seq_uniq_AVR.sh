...
OUTPUT_ROOT="/home/adminbcc/Dataset_AVR/Resultados_seqUnica"
TMPDIR=$(mktemp -d)
...
process_one_ll() {
  ...
  while IFS= read -r line; do
    ...
    # Converte a linha para o formato do LLVM 20 (New Pass Manager)
    # Ex.: "-mem2reg -instcombine" -> "mem2reg,instcombine"
    local passes
    passes=$(echo "$line" | sed -E 's/^[[:space:]]*-//; s/[[:space:]]+-[[:space:]]*/,/g; s/[[:space:]]+//g')
    ...
    # Gera objeto
    if [ $OK -eq 0 ]; then
      llc-20 -march=avr -mtriple=avr -mcpu=atmega328p"$TMPDIR/saida.ll" -filetype=obj -o "$TMPDIR/saida.o"
    fi
    
    #Gera assembly
    if [ $OK -eq 0 ]; then
      llvm-objdump-20 --no-addresses --no-show-raw-insn --demangle -D --triple=avr --mcpu=atmega328p "$TMPDIR/saida.o" > "$TMPDIR/saida.s"

      if [ -z $MELHOR_PASSE ]; then
        ...
        SAIDA_SIZE=$(llvm-size-20 "$OUTPUT_DIR/saida.o" |tail -n1|cut -f4)
    ...
    if [ $OK -eq 0 ]; then
      echo echo "OK  [$(printf "%03d" "$i")] $passes"
    else
      echo echo "ERR [$(printf "%03d" "$i")] $passes"
    fi
