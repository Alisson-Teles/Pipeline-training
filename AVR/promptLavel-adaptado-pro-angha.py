...
def extract_num_ll_from_csv_path(rel_path: str) -> Optional[str]:
    """
    Extrai o nome REAL do arquivo .ll.
    Remove prefixos como 'Resultado', 'Resultado_', etc.
    Exemplo:
      Resultado_extr_foo.ll -> extr_foo.ll
      Resultado00051.ll -> 00051.ll
      extr_bar.ll -> extr_bar.ll
    """
    # Captura qualquer nome que termine com .ll
    m = re.search(r"([^/]+\.ll)", rel_path)
    if not m:
        return None

    ll_name = m.group(1)

    # Remove prefixos conhecidos (Csmith e AnghaBench)
    ll_name = re.sub(r"^Resultado_?", "", ll_name)   # remove "Resultado_" ou "Resultado"
    ll_name = re.sub(r"^Resultado\d+\.?", "", ll_name)  # remove "Resultado00051.ll" → "00051.ll"
...
def find_result_dir(resultados_root: Path, ll_name: str) -> Optional[Path]:
    """
    Encontra a subpasta que termina exatamente com o nome do arquivo .ll
    Ex:
      ll_name = "00051.ll"
      procura: Resultado00051.ll

      ll_name = "extr_libcommoncache.c_h2o_cache_get_duration.ll"
      procura: Resultado_extr_libcommoncache.c_h2o_cache_get_duration.ll
    """
    # busca direta
    for p in resultados_root.iterdir():
        if p.is_dir() and p.name.endswith(ll_name):
            return p

    # busca recursiva mais ampla (fallback)
...
def main():
    ...
    parser.add_argument("--ir_dirname", type=str, default="angha-ll", help="dir com os .ll não otimizados")
    parser.add_argument("--resultados_dirname", type=str, default="Resultados_seqUnica", help="dir com as subpastas Resultado*.ll")

    parser.add_argument("--arch", type=str, default="avr", help='texto curto para o prompt: "stm32" ou "avr"')
    ...
    parser.add_argument("--triple", type=str, default="avr", help="triple para objdump (ex.: 'avr')")
    parser.add_argument("--mcpu", type=str, default="atmega328p", help="mcpu para objdump (ex.: 'atmega328p')")
