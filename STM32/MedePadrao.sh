...
INPUT="$1"
ROOT_DIR="${2:-/home/alisson_eles/tudo}"
...
for optflag in "${OTIMIZACOES[@]}"; do
  ...
  # Gera IR otimizado e objeto (alvo Cortex-M3 como no seu script original)
  opt-20 "$INPUT" -S "$optflag" -o "$ll_out"
  llc-20 -mtriple=thumbv7m-none-eabi -mcpu=cortex-m3 "$ll_out" -filetype=obj -o "$obj_out"
