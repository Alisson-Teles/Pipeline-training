...
def extract_ll_name_from_csv_path(rel_path: str) -> Optional[str]:
    """
    Extrai o *nome do arquivo .ll* a partir da coluna 'arquivo' do CSV.

    Casos suportados:
      - "00001.ll"
      - "arquivos-compilados-stm32-1000/00001.ll"
      - "angha-ll-stm32f1/extr_write_sltg.c_init_sltg_data.ll"
      - "Resultados_seqUnica/Resultadoextr_foo.ll/melhor.ll"
      - "Resultados_seqUnica/Resultado extr_foo.ll/melhor.ll"
      - antigos: "Resultado 00001.ll/melhor.ll"

    Retorna, por exemplo:
      - "00001.ll"
      - "extr_write_sltg.c_init_sltg_data.ll"
    """
    ...
    # 1) Caso AnghaBench / resultado: pai começa com "Resultado"
    #    Ex.: "Resultados_seqUnica/Resultadoextr_foo.ll/melhor.ll"
    parent_name = p.parent.name  # "Resultadoextr_foo.ll"
    m_parent = re.match(r"Resultado\s*(.+\.ll)$", parent_name)
    if m_parent:
        return m_parent.group(1)  # "extr_foo.ll"

    # 2) Caso simples: o próprio caminho aponta pro .ll original
    name = p.name
    if name.endswith(".ll") and name != "melhor.ll":
        # Evita confundir "melhor.ll" com IR original
        return name

    # 3) Formato muito antigo: "Resultado 00001.ll/melhor.ll"
    m = re.search(r"Resultado\s*([^\s/]+\.ll)", rel_path)
    if m:
        return m.group(1)

    # 4) Fallback numérico (00001.ll etc.)
...
def find_result_dir(resultados_root: Path, ll_name: str) -> Optional[Path]:
    """
    Tenta achar a subpasta do resultado para um dado arquivo .ll, por ex.:
      ...
      pastas possíveis:
        - Resultado00001.ll
        - Resultado 00001.ll
        - Resultadoextr_write_sltg.c_init_sltg_data.ll
        - Resultado extr_write_sltg.c_init_sltg_data.ll
    """
    candidates = [
        resultados_root / f"Resultado{ll_name}",
        resultados_root / f"Resultado {ll_name}",
        resultados_root / f"Resultado  {ll_name}",
    ]
    for c in candidates:
    ...
    # busca mais flexível
    for p in resultados_root.glob("Resultado*"):
        if p.is_dir() and p.name.endswith(ll_name):
            return p

    # fallback: vasculhar recursivamente (caso tenha subpastas)
    for p in resultados_root.rglob(f"*{ll_name}"):
        if p.is_dir() and p.name.startswith("Resultado"):
            return p
...
def main():
    ...
    with open(melhores_csv, "r", encoding="utf-8") as fcsv, open(out_jsonl, "w", encoding="utf-8") as fj:
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            ll_name = extract_ll_name_from_csv_path(rel_path)
            if not ll_name:
                sys.stderr.write(
                    f"[WARN] Linha {idx}: não consegui extrair nome de arquivo .ll de '{rel_path}'. Pulando.\n"
                )
                total_skip += 1
                continue

            # Tentativas para achar o IR não otimizado:
            #  1) caminho absoluto exatamente como está no CSV
            #  2) dataset_dir / caminho_relativo do CSV
            #  3) ir_dir / nome_do_arquivo.ll
            ir_path: Optional[Path] = None
            rel_path_obj = Path(rel_path)

            candidates_ir = []
            if rel_path_obj.is_absolute():
                candidates_ir.append(rel_path_obj)
            else:
                candidates_ir.append(dataset_dir / rel_path_obj)
                candidates_ir.append(ir_dir / rel_path_obj.name)
                candidates_ir.append(ir_dir / ll_name)

            for cand in candidates_ir:
                if cand.exists():
                    ir_path = cand
                    break
            ...
            if ir_path is None:
                sys.stderr.write(
                    f"[WARN] Linha {idx}: IR não encontrado para '{ll_name}' "
                    f"(tentativas: {[str(c) for c in candidates_ir]}). Pulando.\n"
                )
                total_skip += 1
                continue

            # Agora procura a pasta de resultado pelo nome do .ll (numérico ou AnghaBench)
            res_dir = find_result_dir(resultados_dir, ll_name)
            if not res_dir:
                sys.stderr.write(
                    f"[WARN] Linha {idx}: pasta de resultado não encontrada p/ '{ll_name}'.\n"
                )
                total_skip += 1
                continue
            ...
            print(f"[TOKENS] {ll_name}: user={n_u} assistant={n_a} total={n_t}")
            token_lines.append(f"{ll_name}\t{n_u}\t{n_a}\t{n_t}")
            total_ok += 1
